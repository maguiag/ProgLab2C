...
int prestamos_buscarID(Prestamos array[], int size, int valorBuscado, int* posicion)                    //cambiar prestamos
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].idPrestamos==valorBuscado)                                                   //cambiar campo ID
            {
                retorno=0;
                *posicion=i;
                break;
...
int prestamos_buscarInt(Prestamos array[], int size, int valorBuscado, int* posicion)                    //cambiar prestamos
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].codLibro==valorBuscado)                                                   //cambiar campo codLibro
            {
                retorno=0;
                *posicion=i;
                break;
...
int prestamos_alta(Prestamos array[], int size, int* contadorID)                          //cambiar prestamos
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        {
            (*contadorID)++;
            array[posicion].idPrestamos=*contadorID;                                                       //campo ID
            array[posicion].isEmpty=0;
            utn_getUnsignedInt("\ngetUnsignedInt: ","\nError",1,sizeof(int),1,1,1,&array[posicion].codLibro);           //mensaje + cambiar campo codLibro
...
//            printf("\n Posicion: %d\n ID: %d\n codLibro: %d\n varFloat: %f\n varString: %s\n varLongString: %s",
//                   posicion, array[posicion].idPrestamos,array[posicion].codLibro,array[posicion].varFloat,array[posicion].varString,array[posicion].varLongString);
            retorno=0;
        }
    }
    return retorno;
...
int prestamos_baja(Prestamos array[], int sizeArray)                                      //cambiar prestamos
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].idPrestamos=0;                                                                   //cambiar campo id
            array[posicion].codLibro=0;                                                               //cambiar campo codLibro
...
int prestamos_bajaValorRepetidoInt(Prestamos array[], int sizeArray, int valorBuscado) //cuando hay que dar de baja todas las posiciones en las que se encuentra ese int
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        {
            if(array[i].idPrestamos==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                array[i].isEmpty=1;
                array[i].idPrestamos=0;                                                                   //cambiar campo id
                array[i].codLibro=0;                                                               //cambiar campo codLibro
...
//            strcpy(bufferString,array[i].varString);                      //cambiar campo varString
            bufferId=array[i].idPrestamos;                                   //cambiar campo id
            bufferIsEmpty=array[i].isEmpty;

            bufferInt=array[i].codLibro;                                //cambiar campo codLibro
...
//            strcpy(bufferLongString,array[i].varLongString);          //cambiar campo varLongString
            ...
            j = i - 1;
//            while ((j >= 0) && strcmp(bufferString,array[j].varString)<0)         //cambiar campo varString                 //Si tiene mas de un criterio se lo agrego, Ej. bufferInt<array[j].codLibro
//            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
//                strcpy(array[j + 1].varString,array[j].varString);          //cambiar campo varString
//                array[j + 1].idPrestamos=array[j].idPrestamos;                                //cambiar campo id
//                array[j + 1].isEmpty=array[j].isEmpty;
//
//                array[j + 1].codLibro=array[j].codLibro;                        //cambiar campo codLibro
...
//            strcpy(array[j + 1].varString,bufferString);                     //cambiar campo varString
            array[j + 1].idPrestamos=bufferId;                                        //cambiar campo id
            array[j + 1].isEmpty=bufferIsEmpty;

            array[j + 1].codLibro=bufferInt;                                                        //cambiar campo codLibro
...
//            else
//                printf("\n ID: %d\n codLibro: %d\n varFloat: %f\n varString: %s\n varLongString: %s",
//                       array[i].idPrestamos,array[i].codLibro,array[i].varFloat,array[i].varString,array[i].varLongString);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
...
int librosSolicitados(Prestamos arrayPrestamo[], Socios arraySocio[], Libros arrayLibro[], int len, int idSocio)
{
    ...
    for(i=0;i<len; i++)
    {
        if(arrayPrestamo[i].codSocio == idSocio)
        {
            printf("\n ID: %d\n codAutor: %d\ntitulo: %s\n",
                   arrayLibro[i].idLibro,arrayLibro[i].codAutor,arrayLibro[i].titulo);
            ret = 0;
        }
    }
    return ret;
