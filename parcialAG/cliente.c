...
int cliente_mostrarDebug(Cliente* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        for(i=0;i<limite;i++)
        {
            printf("[DEBUG] - %d - %s - %d\n",array[i].idCliente, array[i].apellido, array[i].isEmpty);
        }
    }
    return retorno;
}
...
int cliente_alta(Cliente* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        if(i >= 0)
        {
            if(!getValidString("\nIngrese el nombre ","\nNo es un nombre valido","El maximo es de 50",auxNombre,50,2))
            {
...
/** \brief Ordena a los clientes de menor a mayor o de mayor a menor segun el "orden"
 *
 * \param array Cliente* puntero al array
 * \param limite int limite definido para el cliente
...
/*int menu(int* opcion)
{
    ...
    {   system("clear");//limpia pantalla
        printf("1- ALTA de un Cliente\n");
        printf("2- MODIFICACION de un Cliente\n");
